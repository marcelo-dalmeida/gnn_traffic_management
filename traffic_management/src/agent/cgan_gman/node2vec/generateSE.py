...
def process_adjacency_graph_file():
    ...
    def read_graph():
        ...
        adj_file = os.path.join(
            config.ROOT_DIR, config.PATH_TO_DATA, 'original_id_adj.txt')
    ...
    def save_adjacency_graph(adjacency_graph):
        ...
        with open(file, 'w') as handle:
            handle.write('\n'.join(str(line)
                         for line in space_separated_values))

    net_file = os.path.join(
        config.ROOT_DIR, config.PATH_TO_SCENARIO, config.SCENARIO.NET_FILE)
